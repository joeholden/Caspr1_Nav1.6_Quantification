...
def process_all_rois_for_an_image(caspr_image_path, nav_image_path):
    ...
    for r in range(number_of_rois):
        ...
        try:
            ...
            left_perinode_max_index = np.where(s_green == max(s_green[0:caspr_trough_index[0][0]]))[0][0]
            ...
            right_perinode_max_index = np.where(s_green == max(s_green[caspr_trough_index[0][0]:]))[0][0]
            ...
            nav_peak_x = [nav_peak_index[0] / PIXEL_RESOLUTION]
            nav_peak_y = [s_red[nav_peak_index][0]]

            # Get Node and Perinode Measurements
            ...
            if keep_datapoint:
                ...
                ax.plot(x_green / PIXEL_RESOLUTION, s_green, label='Caspr1', color='#0e1d35')
                # ax.scatter(x_green / PIXEL_RESOLUTION, s_green, color='orange')
                # ax.plot(x_green / PIXEL_RESOLUTION, y_green, color='red')
                ax.plot(x_red / PIXEL_RESOLUTION, s_red, label='Nav 1.6', color='#dddee5')
                ax.scatter(critical_points_x, critical_points_y, color='#1167b1', label='Caspr1 Critical Points')
                ax.scatter(nav_peak_x, nav_peak_y, color='#d2b48c', label='Nav 1.6 Critical Point')
                ax.plot(x_green / PIXEL_RESOLUTION, np.full(x_green.shape, threshold), color='green',
                        label='Threshold')
                ax.set_ylabel('8-bit Intensity', fontsize=14)
                ax.set_xlabel('Distance in Microns', fontsize=14)
                ax.set_title(f'Caspr1 & Nav1.6 Intensity Distributions\n ROI:{roi_name_1}', fontsize=16)

                ax.legend(facecolor='#7c8594')
    ...
    try:
        df.to_excel(
            f'Excel Output/individual images/{caspr_image_path.split("/")[-1].strip(".png").strip("C1")}_Results.xlsx')
    except UnboundLocalError:
        pass
...
def process_animal(animal_identity, eye_side):
    ...
    for (caspr, nav) in zipped_caspr_nav_filenames:
        ROI_folder_name = f'ROI#{caspr.strip(".png").replace("C1_", "")}'
